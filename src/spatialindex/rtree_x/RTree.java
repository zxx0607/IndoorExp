...
public class RTree implements ISpatialIndex
{
	...
	public RTree(PropertySet ps, IStorageManager sm, double[][] dismatrix)
	{
		m_rwLock = new RWLock();
		...
		m_treeVariant = SpatialIndex.RtreeVariantQuadratic; //R-tree type
		m_fillFactor = 0.7f;
		m_indexCapacity = 100;
		m_leafCapacity = 100;
	...
	protected void insertData_impl(byte[] pData, Region mbr, int id)
	{
		...
		Node l = root.chooseSubtree(mbr, 0, pathBuffer, pData);
	...
	protected void insertData_impl(byte[] pData, Region mbr, int id, int level, boolean[] overflowTable)
	{
		...
		Node n = root.chooseSubtree(mbr, level, pathBuffer, pData);
	...
	public int[] getLeafChildrenNum(){  //level1 
		
		Node n = readNode(m_rootID);
		int i=0;		
		int childrennum[] = new int[n.getChildrenCount()];
		for(i=0;i<n.getChildrenCount();i++)
		{
			Node n1 = readNode(n.getChildIdentifier(i));
			childrennum[i] = n1.getChildrenCount();
		//	IShape is = n1.getShape();
		//	System.out.println(childrennum[i] );
		}
		
		return childrennum;
	}
	
	public Region[] getLeafRegion(){	//level3
		Node n = readNode(m_rootID);
		System.out.println(n.m_nodeMBR.toString());
		System.out.println(m_stats.getNumberOfNodesInLevel(0));
		System.out.println(n.getChildrenCount());
		int i,j,k;
		k=0;
		Region leafreg[] = new Region[m_stats.getNumberOfNodesInLevel(0)];
		int childcount = 0;
		
		for(i=0;i<n.getChildrenCount();i++)
		{
			Node n1 = readNode(n.getChildIdentifier(i));
//			for(int j=0; j<n1.getChildrenCount(); j++){
//				System.out.println(n1.m_pMBR[j].toString());
//			}
			for(j=0; j<n1.getChildrenCount(); j++){
				Node n2 = readNode(n1.getChildIdentifier(j));
				leafreg[k] = n2.getShape().getMBR();
				System.out.println("leafnum:"+k+" region:"+n2.getShape().toString());
				k++;
				childcount += n2.getChildrenCount();
			}
		//	System.out.println();
		//	System.out.println(n1.getShape().toString());
		}
		System.out.println(childcount);
		Node nt = readNode(readNode(n.getChildIdentifier(0)).getChildIdentifier(0));
		System.out.println(nt.getShape().toString());
		for(i=0; i<nt.getChildrenCount(); i++)
		{
			System.out.println("id: "+nt.getChildIdentifier(i)+"region: "+nt.m_pMBR[i].toString());
		}
	...
	public int[] getLeafID(){//获取一个叶子节点中存放的全部patition的id数组
		...
		for(i=0;i<n.getChildrenCount();i++)
		{
			...
			for(j=0; j<n1.getChildrenCount(); j++){
			//	Node n2 = readNode(n1.getChildIdentifier(j));
				LeafID[k] = n1.getChildIdentifier(j);
				k++;
			}
		}
	//	System.out.println("k"+k);
//		for(i=0;i<LeafID.length;i++)
//			System.out.println("id: "+i+" id: "+LeafID[i]);
	...
	public int[] getChildID(int nodeid){//根据一个节点的ID，返回一个包括其全部子节点的ID数组
		...
		int i;
		for(i=0; i<n.getChildrenCount(); i++){
			childid[i] = n.getChildIdentifier(i);
		}
		return childid;
	}
